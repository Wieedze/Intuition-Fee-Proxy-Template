...
- **`IntuitionFeeProxyV2Sponsored.sol`** — hérite V2, ajoute un pool partagé (`sponsorPool` + `fundPool` / `reclaimFromPool` admin-gated), rate limits per-user (`maxClaimPerTx` cap, `maxClaimsPerDay` tumbling 24h window), sponsored metrics additives. Storage ERC-7201 namespaced (`keccak256("intuition.feeproxy.sponsored.v1")`). User-side seulement — pas de `depositFor`, l'admin ne peut jamais créer d'action on-chain sur une adresse tierce.
