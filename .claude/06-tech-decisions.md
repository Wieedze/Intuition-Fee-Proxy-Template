...
- ✅ `MAX_FEE_PERCENTAGE = 1000` (10% max) — landed via audit fix M-01 sur V1 + V2. Boundary tests tightened.
...
## V2Sponsored (shipped, extra au-delà du bounty)

### Shared-pool model (pas per-user)
- ✅ Un seul slot `sponsorPool` funded par l'admin via `fundPool()` payable
- ✅ `reclaimFromPool(amount, to)` admin-gated pour récupérer les fonds non consommés
- ❌ Pas de per-user credit (`sponsoredCredit[user]` mapping supprimé) — simplification UX
- ❌ Pas de `depositFor*` admin (D3 removal) — l'admin ne peut jamais créer d'action on-chain sur une adresse tierce. User-initiated seulement.

### Rate limits per-user
- ✅ `maxClaimPerTx` (cap par tx) + `maxClaimsPerWindow` (count per user/window) + `maxClaimVolumePerWindow` (TRUST cumulé per user/window) + `claimWindowSeconds` (fenêtre configurable, défaut 86400)
- ✅ `setClaimLimits(maxPerTx, maxPerWindow, maxVolumePerWindow, windowSec)` admin-gated — tous les 4 doivent être strictement > 0 (pas de mode "unlimited")
- ✅ `getClaimStatus(user)` → `(claimsUsed, volumeUsed, windowResetsAt)` view pour la webapp / les dApps consommatrices
- ✅ Tiering (free/pro/premium) : déployer un proxy par tier plutôt que de tracker les tiers on-chain — isolation complète des pools et limites

### Storage
- ✅ Namespaced ERC-7201 : `keccak256("intuition.feeproxy.sponsored.v1")` avec formule canonique `& ~bytes32(uint256(0xff))`
- ✅ Isolé de V2 (pas de slot clash sur upgrade)

### Factory two-channel
- ✅ `enum ProxyChannel { Standard, Sponsored }` dans `createProxy(..., channel)`
- ✅ `setImplementation` (standard) + `setSponsoredImplementation` (sponsored), les deux owner-gated
- ✅ `ProxyCreated` event inclut `name` + `channel`
...
- [ ] Factory permissionless (anyone may deploy)
- [ ] Suppression `receive()` et `_transferFee`
- [ ] Events compatibilité (backward-compat)
- [ ] V2Sponsored shared-pool + no `depositFor` admin
- [ ] Gouvernance emergency admin : option A (whitelistedAdmins) / B (Intuition multi-sig dédié) / C (PAUSER_ROLE AccessControl) — bloque I-02 Pausable + C-01 F2 freeze-versioning
