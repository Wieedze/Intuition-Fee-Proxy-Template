...
async function main() {
  ...
  // Only the Factory is written to the webapp env. Standard / sponsored
  // impls are read live from the Factory — no need to cache them here.
  const envBlock = [
    ...
    `VITE_MULTIVAULT_ADDRESS=${multiVault}`,
    "",
  ].join("\n");

  let finalEnv = envBlock;
  if (fs.existsSync(envPath)) {
    // Preserve other keys (e.g. VITE_WALLETCONNECT_PROJECT_ID) and scrub any
    // legacy impl-address env lines left over from earlier deploys.
    ...
    const filtered = existing
      ...
      .filter(
        (line) =>
          ...
          !line.startsWith("VITE_MULTIVAULT_ADDRESS=") &&
          ...
          !line.startsWith("# (overwrite freely"),
      )
      .join("\n")
      .trim();
